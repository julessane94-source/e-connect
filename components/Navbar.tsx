...
export default function Navbar() {
  ...
  const menuItems = [...publicItems, ...institutionItems, ...workspaceItems];
            ...
            <div className="hidden min-w-0 items-center gap-1 lg:flex">
              ...
              <div className="relative">
                ...
                >
                  <Info size={18} />
                  Institution
                  <ChevronDown className={`h-4 w-4 transition ${isInstitutionOpen ? "rotate-180" : ""}`} />
                </button>
                <AnimatePresence>
                  {isInstitutionOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: 8 }}
                      className="absolute right-0 top-full mt-2 w-56 rounded-lg border border-slate-200 bg-white p-2 shadow-lg dark:border-slate-800 dark:bg-slate-950"
                    >
                      {institutionItems.map((item) => (
                        <Link
                          key={item.href}
                          href={item.href}
                          onClick={() => setIsInstitutionOpen(false)}
          ...
          >
            ...
            <div className="fixed right-0 top-0 h-full w-[min(88vw,360px)] overflow-y-auto bg-white p-6 shadow-2xl dark:bg-gray-900">
              ...
              <nav className="space-y-5">
                <MobileMenuSection title="Navigation" items={publicItems} pathname={pathname} onNavigate={() => setIsOpen(false)} />
                <MobileMenuSection title="Institution" items={institutionItems} pathname={pathname} onNavigate={() => setIsOpen(false)} />
...
function MobileMenuSection({
  ...
  return (
    <div>
      <p className="mb-2 px-1 text-xs font-semibold uppercase tracking-wide text-gray-400">{title}</p>
